...
import burp.vocabularies.Observable;
...
public class LogicalSourceFactory {
	...
	public static LogicalSource createNetconfQuerySource(Resource ls) {
		...
		source.endpoint = socketAddress.getProperty(Observable.addressValue).getLiteral().getString();
		...
		Resource accountAuthentication = serverAccount.getPropertyResourceValue(Observable.hasFacet);
		source.password = accountAuthentication.getProperty(Observable.password).getLiteral().getString();
